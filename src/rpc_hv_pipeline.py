...
Run the RPC HV-scan workflow with regime-aware outputs, optional per-HV
cut estimation, and optional per-tracklet diagnostic tables.

This version passes --write-csv and --write-ndjson to the Option-C producer,
so the pipeline can save the per-tracklet information needed for angular
...
def resolve_script(path_value: Optional[str], default_name: str) -> str:
    if path_value:
        return str(Path(path_value))
    return str(SCRIPT_DIR / default_name)


def run(cmd: List[str], logfile: Optional[Path] = None) -> None:
    print(" ".join(cmd))
    t0 = time.time()
...
def infer_producer_mode(script_path: str) -> str:
    ...
    if "optionc" in name or "nch" in name or name in {"rpc_tracklet_efficiency.py", "rpc_tracklets_efficiency.py"}:
        return "optionc"
    return "v2"


def parse_z_list(z_list: Optional[List[float]], n: int) -> Optional[List[str]]:
    if z_list is None:
        return None
    if len(z_list) != n:
...
def load_cuts_json(path: Optional[str]) -> Dict[str, object]:
    if not path:
        return {}
...
def resolve_hv_cuts(hvtag: str, args: argparse.Namespace, cuts_db: Dict[str, object]) -> Dict[str, object]:
    ...
    return {
        ...
        "theta_max_deg": None
        if entry.get("theta_max_deg") is None
        else float(entry.get("theta_max_deg")),
...
def maybe_generate_cuts(
    args: argparse.Namespace,
    mode: str,
    chamber_bases: List[Path],
    z_strs: Optional[List[str]],
    out_dir: Path,
) -> Optional[str]:
    ...
    cmd = [
        ...
        mode,
        "--min-samples",
        str(args.cuts_min_samples),
        "--dx-nsigma",
        str(args.cuts_dx_nsigma),
        "--dy-nsigma",
        str(args.cuts_dy_nsigma),
        "--dt-nsigma",
        str(args.cuts_dt_nsigma),
        "--min-tol-x",
        str(args.cuts_min_tol_x),
        "--max-tol-x",
        str(args.cuts_max_tol_x),
        "--min-tol-y",
        str(args.cuts_min_tol_y),
        "--max-tol-y",
        str(args.cuts_max_tol_y),
        "--min-tol-t",
        str(args.cuts_min_tol_t),
        "--max-tol-t",
        str(args.cuts_max_tol_t),
        "--percentile",
        str(args.cuts_percentile),
        "--angle-max-deg",
        str(args.cuts_angle_max_deg),
    ]
...
def main() -> None:
    ...
    ap.add_argument("--auto-cuts", action="store_true", help="Estimate per-HV cuts before the HV loop.")
    ap.add_argument(
        "--auto-cuts-script",
        default="rpc_estimate_tracklet_cuts.py",
    ...
    # New pass-through switches for diagnostic data products.
    ap.add_argument(
        "--write-csv",
        action="store_true",
        help="Ask the Option-C producer to write per-tracklet CSV diagnostics.",
    )
    ap.add_argument(
        "--write-ndjson",
        action="store_true",
        help="Ask the Option-C producer to write per-tracklet NDJSON diagnostics.",
    )
    ap.add_argument(
        "--no-diagnostic-hists",
        action="store_true",
        help="Ask the Option-C producer not to write residual/angular ROOT histograms.",
    )
    ...
    hv_dirs = sorted(
        [d for d in chamber_bases[0].glob("HV*") if d.is_dir()],
    ...
    for hvdir in hv_dirs:
        ...
        print(
            "Using cuts: "
            f"tol_x={hv_cuts['tol_x']:.3f}, "
            f"tol_y={hv_cuts['tol_y']:.3f}, "
            f"tol_t={hv_cuts['tol_t']:.3f} "
            f"[{hv_cuts['source']}]"
        )
        ...
        if mode == "v2":
            ...
            # New diagnostic pass-through options.
            if args.write_csv:
                cmd += ["--write-csv"]
            if args.write_ndjson:
                cmd += ["--write-ndjson"]
            if args.no_diagnostic_hists:
                cmd += ["--no-diagnostic-hists"]
        ...
        write_metadata(
            meta_out,
            {
                ...
                "write_ndjson": bool(args.write_ndjson),
                "diagnostic_hists": not bool(args.no_diagnostic_hists),
            },
        )
    ...
    cmd_hv = [
        ...
        args.hvscript,
        "--glob",
        str(sum_dir / f"summary_HV*_{args.regime}.json"),
        "--out",
        str(plot_out),
    ]
    if args.fit:
    ...
    if args.fit:
        print(f"Fit: {fit_out}")
    if args.write_csv:
        print(f"CSV diagnostics: {map_dir}/tracklets_full_HV*_{args.regime}.csv")
    if args.write_ndjson:
        print(f"NDJSON diagnostics: {map_dir}/tracklets_HV*_{args.regime}.ndjson")
    if not args.no_diagnostic_hists:
        print("ROOT diagnostic histograms: h_dx, h_dy, h_theta_x, h_theta_y, h_theta_x_theta_y, ...")
