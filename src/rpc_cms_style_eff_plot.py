...
import matplotlib
matplotlib.use("Agg")  # avoids Qt/Wayland GUI warnings
...
def fit_sigmoid(hv, eff):
    ...
    try:
        ...
        return tuple(map(float, p0))  # still fine for plotting
...
def load_series(glob_pattern: str, use_roi: bool):
    ...
    for p in paths:
        ...
        if "hv_kv" in d:
            ...
            hv_kv = infer_hv_from_filename(p)

        # Eff/err in your schema:
        if use_roi:
...
def main():
    ...
    for spec in args.series:
        ...
        ax.plot(hv_fine, 100.0 * sigmoid(hv_fine, eps0, eps_max, hv50, k), linewidth=1.5, alpha=0.8)
